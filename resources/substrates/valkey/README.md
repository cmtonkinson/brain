# Valkey Substrate
Valkey-backed _Substrate_ _Resource_ used by the Cache Authority _Service_ for scoped cache storage, queue operations, and substrate liveness checks.

------------------------------------------------------------------------
## What This Component Is
`resources/substrates/valkey/` provides the Layer 0 Valkey integration for Brain:
...
- CAS resolves component settings via `resolve_valkey_settings(...)`.
- CAS constructs the substrate with `ValkeyClientSubstrate(...)`.
- CAS performs cache and queue operations through the `ValkeySubstrate`
  protocol (`set_value`, `get_value`, `delete_value`, `push_queue`,
  `pop_queue`, `peek_queue`, `ping`).
- Valkey failures are surfaced to CAS for mapping into service-level structured
  dependency errors.
