# Brain Worker
The Brain Worker is an L2 actor process that claims queued job executions
from the Job Service and runs them by invoking capabilities through the
Brain SDK.
...
3. Back-pressures when the pool is saturated (all `max_workers` slots
   occupied) by sleeping `0.25 s` before retrying.
...
1. `client.invoke_capability` — runs the capability identified in the
...
| `worker.principal` | SDK envelope `principal` field |
...
Worker Actor is a Layer 2 Actor. It owns no Service or Resource
components.
...
Coverage:
- Success path: `invoke_capability` then `job_complete_execution` called.
