...
from services.control.commitment.interfaces import CommitmentRepository
from services.control.commitment.service import CommitmentService
from services.control.commitment.validation import (
    CommitmentIdRequest,
...
from services.action.language_model.service import LanguageModelService, ReasoningLevel
...
class DefaultCommitmentService(CommitmentService):
    """Default Commitment Service implementation with Postgres authority."""
    ...
    def update_commitment(
        ...
        *,
        ...
        try:
            ...
            job_link = self._sync_follow_up_after_update(meta=meta, commitment=updated)
            refreshed = (
    ...
    def deliver_review(
        ...
        if self._attention_router_service is None:
            return failure(
                ...
                errors=[
                    dependency_error(
                        "Attention Router Service is not available",
        ...
        router_env = self._attention_router_service.route_notification(
            meta=meta,
            actor="operator",
    ...
    def _ensure_follow_up_job_for_record(
        ...
        create_env = self._job_service.create_job(
            ...
            job_action={
                ...
                "capability_id": self._settings.follow_up_capability_id,
    ...
    def _sync_follow_up_after_update(
        self, *, meta: EnvelopeMeta, commitment: CommitmentRecord
    ) -> CommitmentJobLink | None:
        return self._ensure_follow_up_job_for_record(meta=meta, commitment=commitment)
    ...
    def _notify_missed_commitment(
        ...
        if self._attention_router_service is None:
            return False
        notify_env = self._attention_router_service.route_notification(
            meta=meta,
            actor="operator",
    ...
    def _check_dedupe(
        ...
        *,
        ...
        Gracefully degrades: LMS unavailability, disabled config, empty open set,
        parse errors, or low confidence all return ``(None, None, None)`` and
        allow creation to proceed.
        """
        if self._language_model_service is None or not self._settings.dedupe_enabled:
        ...
        try:
            ...
            if chat_env.errors or chat_env.payload is None:
                _LOGGER.warning("Dedupe LMS call failed; skipping dedupe check")
                return None, None, None
