...
from services.reason.ingestion.component import SERVICE_COMPONENT_ID
from services.reason.ingestion.config import IngestionServiceSettings
...
class DefaultIngestionService(IngestionService):
    """Default Ingestion Service implementation with Postgres-backed state."""
    ...
    def submit_ingestion(
        ...
        *,
        ...
        if parse_error:
            self._repository.create_ingestion(
    ...
    def run_store_stage(
        ...
        *,
        ...
        """Execute the store stage — write raw artifact to Object, record outcome."""
        validate_meta(meta)
        now = datetime.now(UTC)
    ...
    def run_anchor_stage(
        ...
        *,
        ...
        """Execute the anchor stage — write anchor notes to vault via Vault."""
        validate_meta(meta)
        now = datetime.now(UTC)
        ...
        *,
        ...
        if self._object_service is None:
            return self._fail_indexing_dependency(
                ...
                message="Object Service is not available",
            )
        ...
        if self._language_service is None:
            return self._fail_indexing_dependency(
                meta=meta,
                indexing_run_id=indexing_run_id,
                message="Language Service is not available",
            )
        ...
        active_spec_env = self._embedding_service.get_active_spec(meta=svc_meta)
        if (
            ...
            error_text = (
                _join_errors(active_spec_env.errors) or "Embedding active spec missing"
            )
        ...
        for anchor in anchors:
            get_env = self._object_service.get_object(
            ...
            if (
                ...
                failed_count += 1
                errors.append(
                    f"object_key={anchor.normalized_object_key} error=Object get_object failed"
                )
                continue
            ...
            source_env = self._embedding_service.upsert_source(
                meta=svc_meta,
                canonical_reference=(
            ...
            chunk_env = self._embedding_service.upsert_chunks(
            ...
            embed_env = self._language_service.embed_batch(
                meta=svc_meta,
            ...
            vector_env = self._embedding_service.upsert_embedding_vectors(
    ...
    def _enqueue_indexing_job(
        ...
        *,
        ...
        create_env = self._job_service.create_job(
            ...
            job_action={
                "type": "op_invocation",
                "op_id": "ingestion-index-anchored",
                "input_payload": {
                    "ingestion_id": ingestion_id,
    ...
    def _run_store_stage_inline(
        ...
        *,
        ...
        if result.status == StageArtifactStatus.failed:
            ...
            return failure(
                meta=meta, errors=[internal_error(result.error or "store stage failed")]
            )
    ...
    def _execute_store_stage(
        ...
        *,
        ...
        """Replay-path store stage: re-validate an existing raw artifact in Object.
        ...
        store outcome and stat the object in Object to confirm it still exists.
        """
        if self._object_service is None:
            raise RuntimeError("Object Service is not available")

        object_meta = new_meta(
        ...
        stat_envelope = self._object_service.stat_object(
            meta=object_meta, object_key=object_key
        )
        if stat_envelope.errors:
            error_text = f"raw artifact no longer available in Object: {object_key}"
    ...
    def _execute_store_stage_inline(
        ...
        *,
        ...
        Writes raw artifact to Object (or validates existing key), then records
        ...
        if payload is not None:
            ...
            # Check whether this content already exists in Object (dedupe)
            put_envelope = self._object_service.put_object(
                meta=object_meta,
                content=payload,
                extension=ext,
                content_type=ct,
            ...
            if (
                ...
                error_text = (
                    ...
                    else "Object put_object returned no payload"
                )
                self._repository.create_stage_artifact_outcome(
                    ingestion_id=record.id,
                    stage="store",
        ...
        stat_envelope = self._object_service.stat_object(
            meta=object_meta, object_key=existing_object_key
        )
        if stat_envelope.errors:
    ...
    def _execute_extract_stage(
        ...
        *,
        ...
        for raw_outcome in eligible:
            ...
            get_envelope = self._object_service.get_object(
                meta=object_meta, object_key=raw_outcome.object_key
            )
            if (
                get_envelope.errors
                ...
                err = f"object_key={raw_outcome.object_key} error=Object get_object failed"
                errors.append(err)
                failed += 1
                self._repository.create_stage_artifact_outcome(
                    ingestion_id=record.id,
            ...
            for extractor in extractors:
                ...
                for artifact in extracted_list:
                    ...
                    put_env = self._object_service.put_object(
                        meta=object_meta,
                    ...
                    if (
                        ...
                        err = f"object_key={raw_outcome.object_key} error=Object put_object failed for extracted artifact"
                        errors.append(err)
                        failed += 1
                        self._repository.create_stage_artifact_outcome(
                            ingestion_id=record.id,
    ...
    def _execute_normalize_stage(
        ...
        *,
        ...
        for extracted_outcome in eligible:
            ...
            get_envelope = self._object_service.get_object(
                meta=object_meta, object_key=extracted_outcome.object_key
            )
            if (
                get_envelope.errors
                or get_envelope.payload is None
                ...
                err = f"object_key={extracted_outcome.object_key} error=Object get_object failed"
                errors.append(err)
                failed += 1
                self._repository.create_stage_artifact_outcome(
                    ingestion_id=record.id,
            ...
            for normalizer in normalizers:
                ...
                for artifact in normalized_list:
                    ...
                    if (
                        ...
                        err = f"object_key={extracted_outcome.object_key} error=Object put_object failed for normalized artifact"
                        errors.append(err)
                        failed += 1
                        self._repository.create_stage_artifact_outcome(
                            ingestion_id=record.id,
    ...
    def _execute_anchor_stage(
        ...
        *,
        ...
        """Core anchor-stage logic: write vault anchor notes via Vault public API."""
        if self._vault_service is None:
            raise RuntimeError("Vault Service is not available")
        if self._object_service is None:
            raise RuntimeError("Object Service is not available")
        ...
        for index, outcome in enumerate(eligible, start=1):
            ...
            get_env = self._object_service.get_object(
                meta=svc_meta, object_key=outcome.object_key
            )
            if (
                get_env.errors
                or get_env.payload is None
                ...
                err = f"object_key={outcome.object_key} error=Object get_object failed"
                errors.append(err)
                failed += 1
                self._repository.create_stage_artifact_outcome(
                    ingestion_id=record.id,
            ...
            try:
                ...
                if not note_created:
                    ...
                    create_env = self._vault_service.create_file(
                        meta=svc_meta,
                        file_path=note_path,
                    ...
                    if create_env.errors:
                        ...
                        append_env = self._vault_service.append_file(
                            meta=svc_meta,
                            file_path=note_path,
                            content=f"\n\n---\n\n{section}",
                        )
                        if append_env.errors:
                            ...
                            err = f"object_key={outcome.object_key} error=Vault create/append file failed"
                            errors.append(err)
                            failed += 1
                            self._repository.create_stage_artifact_outcome(
                                ingestion_id=record.id,
                    ...
                    if append_env.errors:
                        ...
                        err = f"object_key={outcome.object_key} error=Vault append_file failed"
                        errors.append(err)
                        failed += 1
                        self._repository.create_stage_artifact_outcome(
                            ingestion_id=record.id,
...
def _filename_for_ingestion(ingestion_id: str, ext: str) -> str:
    """Build a stable original_filename for Object from ingestion context."""
