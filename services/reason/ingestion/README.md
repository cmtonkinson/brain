# Ingestion Service
Control *Service* that owns content-ingestion attempts, stage execution state, artifact lineage, provenance, anchor-note linkage, and derived indexing handoff records.

------------------------------------------------------------------------
## What This Component Is
`services/reason/ingestion/` is the authoritative Tier 2 *Service* for deterministic content ingestion in Brain's Control System.
...
Ingestion Service is a Control-System *Service* (`tier=2`, `plane="reason"`). It owns the `service_ingestion` Postgres schema exclusively.
...
* Blob persistence routes through `ObjectService` public APIs only.
* Vault anchor writes route through `VaultService` public APIs only.
* Stage and indexing orchestration routes through `JobService` and Execution op invocation.
* Derived indexing uses `UtilityService`, `LanguageService`, and `EmbeddingService` public APIs only.
...
Component tests live in `services/reason/ingestion/tests/`.
