"""Ingestion Service-owned Postgres runtime wiring."""
...
class IngestionPostgresRuntime:
    """Concrete Ingestion Service-owned handle for schema-scoped Postgres access."""
    ...
    @classmethod
    def from_settings(cls, settings: CoreRuntimeSettings) -> IngestionPostgresRuntime:
        """Build Ingestion DB runtime from typed application settings."""
        postgres_config = resolve_postgres_settings(settings)
        engine = create_postgres_engine(postgres_config)
        factory = create_session_factory(engine)
        schema = ingestion_postgres_schema()
        return cls(
            engine=engine,
...
def ingestion_postgres_schema() -> str:
    """Resolve canonical Ingestion Service schema name from component identity."""
