...
class _FakeObjectService:
    """Object fake with configurable failure modes."""
    ...
    def put_object(
        ...
        *,
        ...
        if self.fail_put:
            return failure(meta=meta, errors=[internal_error("Object put failed")])
...
class _FakeVaultService:
    ...
    def create_file(self, *, meta, file_path: str, content: str):
        ...
        if self.fail_create:
            return failure(meta=meta, errors=[internal_error("Vault create failed")])
    ...
    def append_file(self, *, meta, file_path: str, content: str):
        ...
        if self.fail_append:
            return failure(meta=meta, errors=[internal_error("Vault append failed")])
...
def _make_service(
    *,
    ...
    object_service = object_service or _FakeObjectService()
    vault_service = vault_service or _FakeVaultService()
    ...
    return svc, repo, object_service, vault_service, job_service


def _submit(svc, repo, object_service, *, content: bytes = b"hello world"):
...
class TestNullPayloadGuards:
    def test_extract_stage_oas_get_null_payload_records_failed_outcome(self) -> None:
        ...
        object_service = _FakeObjectService()
        svc, repo, object_service, vault_service, job_service = _make_service(
            repo=repo, object_service=object_service, with_extractors=True
        )
        # Submit creates a store outcome; then manually tweak Object to return null payload
        ingestion_id = _submit(svc, repo, object_service)
        object_service.return_null_payload_on_get = True
    ...
    def test_extract_stage_oas_put_null_payload_records_failed_outcome(self) -> None:
        ...
        ingestion_id = _submit(svc, repo, object_service)
        object_service.return_null_payload_on_put = True
    ...
    def test_enqueue_advance_job_null_payload_marks_ingestion_failed(self) -> None:
        ...
        svc, repo, object_service, vault_service, _ = _make_service(
            repo=repo, job_service=job_service
        )
...
class TestAnchorWriteOrdering:
    def test_vault_write_failure_removes_db_anchor_so_retry_can_proceed(self) -> None:
        """When Vault write fails, anchor DB record must be deleted so retry re-attempts."""
        repo = _FakeRepository()
        object_service = _FakeObjectService()
        vault_service = _FakeVaultService()
        vault_service.fail_create = True
        vault_service.fail_append = True
        svc, repo, object_service, vault_service, job_service = _make_service(
            repo=repo,
            object_service=object_service,
            vault_service=vault_service,
    ...
    def test_retry_skips_already_anchored_artifact(self) -> None:
        ...
        svc.run_anchor_stage(meta=_meta(), ingestion_id=ingestion_id)
        vault_write_count_after_first = sum(
            len(v) for v in vault_service.files.values()
        )

        # Second anchor run — artifact already anchored, vault must NOT be touched again
        svc.run_anchor_stage(meta=_meta(), ingestion_id=ingestion_id)
        vault_write_count_after_second = sum(
            len(v) for v in vault_service.files.values()
        )
...
class TestConcurrentRetryBlocking:
    ...
    def test_retry_ingestion_stage_blocked_when_stage_running(self) -> None:
        ...
        repo = _FakeRepository()
        svc, repo, object_service, vault_service, job_service = _make_service(repo=repo)
        ingestion_id = _submit(svc, repo, object_service)
...
class TestReplayProvenance:
    def test_store_stage_replay_records_provenance(self) -> None:
        ...
        object_service = _FakeObjectService()
        svc, repo, object_service, vault_service, job_service = _make_service(
            repo=repo, object_service=object_service
        )
        ingestion_id = _submit(svc, repo, object_service)
