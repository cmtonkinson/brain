...
# ---------------------------------------------------------------------------
# In-memory Object fake
# ---------------------------------------------------------------------------


class _FakeObjectService:
    """Minimal Object fake for store-stage testing."""
    ...
    def _make_object_record(self, object_key: str):
        """Mimics Object ObjectRecord (returned by put_object / stat_object)."""
    ...
    def _make_get_result(self, object_key: str, content: bytes):
        """Mimics Object ObjectGetResult (returned by get_object)."""
        from types import SimpleNamespace
...
class _FakeVaultService:
    """Minimal Vault fake for anchor-stage testing."""
...
class _FakeLanguageService:
    """Minimal Language fake for indexing tests."""
...
class _FakeEmbeddingService:
    """Minimal Embedding fake for indexing tests."""
...
def _make_service(
    *,
    ...
    object_service: _FakeObjectService | None = None,
    vault_service: _FakeVaultService | None = None,
    job_service: _FakeJobService | None = None,
    utility_service: _FakeUtilityService | None = None,
    language_service: _FakeLanguageService | None = None,
    embedding_service: _FakeEmbeddingService | None = None,
    ...
    DefaultIngestionService, _FakeRepository, _FakeObjectService, _FakeJobService
]:
    ...
    return svc, repo, object_service, job_service
...
class TestSubmitIngestion:
    ...
    def test_existing_object_key_path(self) -> None:
        svc, repo, obj, _ = _make_service()
        # Pre-populate Object service with a known key
        obj._objects["existing/key.txt"] = b"content"
        env = svc.submit_ingestion(
            meta=_test_meta(),
            source_type="upload",
...
class TestAdvanceIngestion:
    ...
    def test_builtin_text_handlers_advance_to_anchor_and_schedule_indexing(
        ...
        assert _pv(env).status == IngestionStatus.complete
        assert len(vault.files) == 1
        assert jobs.created_jobs[-1]["job_action"]["op_id"] == (
...
class TestAnchorStage:
    def test_noop_anchor_creates_one_skipped_run(self) -> None:
        vault = _FakeVaultService()
        svc, repo, _, _ = _make_service(vault_service=vault)
        record = repo.create_ingestion(
            status=IngestionStatus.running.value,
            source_type="test",
            source_uri=None,
...
class TestIndexAnchoredIngestion:
    def test_indexes_anchor_through_public_service_dependencies(self) -> None:
        ...
        obj = _FakeObjectService()
        utility = _FakeUtilityService()
        lang = _FakeLanguageService()
        emb = _FakeEmbeddingService()
        svc, repo, obj, _jobs = _make_service(
            repo=repo,
            object_service=obj,
