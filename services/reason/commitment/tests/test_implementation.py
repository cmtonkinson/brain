...
from services.effect.relay.domain import RouteNotificationResult
from services.effect.language.domain import ChatResponse
...
class _FakeRelayOutboundService:
    def __init__(self) -> None:
...
class _FakeLmsService:
    """Fake Language Service for dedupe testing."""
    ...
    def chat(self, *, meta, system_prompt="", prompt="", profile="standard"):
        ...
        if self._fail:
            return failure(
                meta=meta,
                errors=[validation_error("Language unavailable", code="LMS_ERROR")],
            )
        return success(
            meta=meta,
...
def _service(
    ...
    attention: _FakeRelayOutboundService | None = None,
    ...
    attention_service = attention or _FakeRelayOutboundService()
    return (
        DefaultCommitmentService(
            ...
            outbound_service=attention_service,
            language_service=lms,
        ),
        repo,
        job_service,
        attention_service,
...
def test_dedupe_skipped_when_language_unavailable() -> None:
    service, repo, _jobs, _attention = _service(lms=None)

    envelope = service.create_commitment(
        meta=_meta(),
        description="No Language available",
    )
...
def test_dedupe_graceful_on_language_error() -> None:
    repo = _FakeRepository()
    now = datetime.now(UTC)
    repo.create_commitment(
    ...
    envelope = service.create_commitment(
        meta=_meta(),
        description="Language fails gracefully",
    )
...
def test_extract_candidates_no_language_returns_empty() -> None:
...
def test_extract_candidates_disabled_returns_empty_without_language_call() -> None:
...
def test_extract_candidates_language_failure_returns_empty() -> None:
    lms = _FakeLmsService(fail=True)
    service, _repo, _jobs, _attention = _service(lms=lms)
