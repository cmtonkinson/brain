"""HTTP route adapter tests for Recall Service."""
...
from services.reason.recall.api import register_routes
...
class _FakeRecallService(RecallService):
    """Programmable Recall fake for route-adapter tests."""
...
def _meta():
    """Build valid envelope metadata for Recall route-test responses."""
    return new_meta(kind=EnvelopeKind.COMMAND, source="test", principal="operator")


def _client() -> tuple[TestClient, _FakeRecallService]:
    """Create one FastAPI test client with the Recall route adapter mounted."""
    app = create_app()
    router = APIRouter()
    service = _FakeRecallService()
