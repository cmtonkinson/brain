...
class OpPolicyInput(BaseModel):
    """Op metadata evaluated by Policy Service for one invocation."""
...
class OpInvocationRequest(BaseModel):
    """Normalized policy input contract for one op invocation."""
...
class PolicyRule(BaseModel):
    """One effective policy rule scoped to a specific op identifier."""
...
class ActorPolicyDefaults(BaseModel):
    """Default per-effect posture for one actor principal."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    read: Literal["allow", "require_approval", "deny"] = "deny"
    write: Literal["allow", "require_approval", "deny"] = "deny"
    execute: Literal["allow", "require_approval", "deny"] = "deny"
    external: Literal["allow", "require_approval", "deny"] = "deny"


class ActorPolicyOverride(BaseModel):
    """Per-op actor override over one actor's defaults."""
...
class ActorPolicyDeclaration(BaseModel):
    """Actor-level default and per-op policy declarations."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    defaults: ActorPolicyDefaults = Field(default_factory=ActorPolicyDefaults)
    overrides: dict[str, ActorPolicyOverride] = Field(default_factory=dict)
