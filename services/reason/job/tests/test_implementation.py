...
from services.reason.job.config import JobServiceSettings
...
from services.reason.job.interfaces import ProviderHealthStatus, ProviderJobPayload
...
class _FakeExecution:
    """Deterministic Execution fake."""

    def __init__(self) -> None:
        self.descriptors: dict[str, OpDescriptor] = {}
        self.invoke_results: dict[str, list[tuple[str, dict[str, Any] | None]]] = {}
        self.invocations: list[tuple[str, dict[str, object], OpInvocationMetadata]] = []
    ...
    def register_descriptor(
        ...
        *,
        ...
        self.descriptors[op_id] = OpDescriptor(
            op_id=op_id,
            kind="native",
            version="1.0.0",
            summary="fake op",
    ...
    def queue_success(
        self, op_id: str, *, output: dict[str, Any] | None = None
    ) -> None:
        self.invoke_results.setdefault(op_id, []).append(("success", output))

    def queue_dependency_failure(self, op_id: str) -> None:
        self.invoke_results.setdefault(op_id, []).append(("dependency", None))

    def queue_policy_failure(self, op_id: str) -> None:
        self.invoke_results.setdefault(op_id, []).append(("policy", None))

    def describe_op(self, *, meta: Any, op_id: str) -> Any:
        descriptor = self.descriptors.get(op_id)
    ...
    def invoke_op(
        ...
        *,
        ...
        self.invocations.append((op_id, input_payload, invocation))
        kind, output = self.invoke_results.get(op_id, [("success", {"ok": True})]).pop(
            0
        )
        if kind == "dependency":
...
def _build_service(
    *,
...
) -> tuple[DefaultJobService, _FakeRepository, _FakeProvider | None, _FakeExecution]:
    repo = repo or _FakeRepository()
    execution = execution or _FakeExecution()
    ...
    return service, repo, provider, execution
...
class TestCreateJob:
    def test_active_job_registers_provider_and_stores_action(self) -> None:
        ...
        execution.register_descriptor("demo-op")
        service, repo, _, _ = _build_service(provider=provider, execution=execution)
...
class TestExecutionDispatch:
    def test_run_now_creates_queued_execution(self) -> None:
        execution = _FakeExecution()
        execution.register_descriptor("demo-op")
        service, repo, _, engine = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
            summary="Dispatch",
            schedule_type="interval",
    ...
    def test_one_time_job_completes_after_worker_reports_success(self) -> None:
        execution = _FakeExecution()
        execution.register_descriptor("demo-op")
        service, repo, _, _ = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
            summary="One time",
    ...
    def test_duplicate_callback_returns_duplicate(self) -> None:
        execution = _FakeExecution()
        execution.register_descriptor("demo-op")
        execution.queue_success("demo-op", output={"ok": True})
        service, _, _, _ = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
            summary="Duplicate",
...
class TestRetryProcessing:
    def test_retry_due_marks_exhausted_execution_failed(self) -> None:
        ...
        service, repo, _, _ = _build_service(settings=settings, execution=execution)
        created = service.create_job(
            meta=_meta(),
            summary="Retry cap",
            schedule_type="interval",
...
class TestConditionalEvaluation:
    def test_true_predicate_dispatches_execution(self) -> None:
        execution = _FakeExecution()
        execution.register_descriptor("predicate-read")
        execution.register_descriptor("demo-op")
        execution.queue_success("predicate-read", output={"status": "ready"})
        service, repo, _, engine = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
            ...
            timezone="UTC",
            definition={
                "predicate_op_id": "predicate-read",
        ...
        assert execution.status == ExecutionStatus.queued
        assert repo.jobs[created.id].last_run_status is None
        # Only predicate evaluated; op dispatch deferred to Worker
    ...
    def test_false_predicate_records_audit_without_dispatch(self) -> None:
        ...
        execution.register_descriptor("predicate-read")
        execution.register_descriptor("demo-op")
        execution.queue_success("predicate-read", output={"status": "waiting"})
        service, repo, _, engine = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
    ...
    def test_side_effecting_predicate_op_fails_closed(self) -> None:
        execution = _FakeExecution()
        execution.register_descriptor(
            "predicate-read",
            approval="always",
        )
        execution.register_descriptor("demo-op")
...
class TestWorkerInterface:
    ...
    def _make_queued_job(
        ...
        *,
        ...
        if definition is None:
            definition = {"interval_count": 1, "interval_unit": "hour"}
        execution_fake = _FakeExecution()
        execution_fake.register_descriptor("demo-op")
        created = service.create_job(
            meta=_meta(),
    ...
    def test_claim_returns_running_execution(self) -> None:
        ...
        assert result.intent.action.op_id == "demo-op"
...
class TestConditionalEvaluationOrder:
    def test_schedule_not_advanced_when_predicate_raises(self) -> None:
        ...
        execution.queue_success("predicate-read", output={"count": "not-a-number"})
        service, repo, _, _ = _build_service(execution=execution)
        created = service.create_job(
            meta=_meta(),
            summary="eval order",
