# Job Service
Control *Service* that owns job intent, schedule state, execution history, and audit
...
`services/reason/job/` is the authoritative Tier 2 *Service* for scheduled work in
...
Job Service is a Control-System *Service* (`tier=2`, `plane="reason"`). It uses
...
* All operator-facing notifications must route through Relay outbound.
* All op invocations must route through Execution and Policy Service.
* Conditional predicate resolution must route through allowed service public APIs or
  Execution read-only ops — never direct internal imports.
...
* `evaluate_conditional_job` is invoked on a cadence (scheduled actor context) to
  evaluate conditional schedules through read-only Execution calls.
...
1. Job Service is built by `build_job_service(settings, execution_service)`,
...
8. Scheduled and run-now executions dispatch through Execution using the
...
* `stalled_execution_threshold_minutes` (default: `60`) — running-state age
  threshold for stalled-execution detection
* `retry_batch_size` (default: `100`) — max executions processed per
  `process_retry_due_jobs` invocation
...
Component tests:
* `services/reason/job/tests/test_domain.py` — domain model invariants and
  discriminated union round-trips
* `services/reason/job/tests/test_validation.py` — request validation models and
  definition validators
* `services/reason/job/tests/test_retry.py` — retry math (backoff, delay, deadline)
* `services/reason/job/tests/test_timing.py` — next-run calculations for all
  schedule types
* `services/reason/job/tests/test_implementation.py` — command/query semantics,
...
* Predicate evaluation must stay read-only: deny ops that require approval
...
* Predicate evaluation separated from execution dispatch — `op_gate.py`
...
* `agent_invoker.py` — direct agent wiring replaced by service public APIs and
  Execution / Policy Service flows
...
* Direct imports from ingestion, attention, commitment, or other service internals —
  replaced by public API calls or op flows
