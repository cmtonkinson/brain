...
class OpInvocationAction(BaseModel):
    """Invoke one registered op with typed input payload."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    type: Literal["op_invocation"] = "op_invocation"
    op_id: str = Field(
        min_length=1,
...
class ConditionalDefinition(BaseModel):
    """Conditional schedule: evaluates a read-only op-backed predicate."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    type: Literal["conditional"] = "conditional"
    predicate_op_id: str = Field(
        min_length=1,
        pattern=r"^[a-z0-9]+(?:-[a-z0-9]+)*$",
    )
