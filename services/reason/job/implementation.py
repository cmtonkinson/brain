...
from services.effect.execution.service import ExecutionService, OpInvocationMetadata
...
from services.reason.job.interfaces import (
    JobProviderAdapter,
    JobRepository,
    ProviderJobPayload,
)
from services.reason.job.retry import compute_retry_at, should_retry
from services.reason.job.service import JobService
from services.reason.job.timing import compute_next_run
from services.reason.job.validation import (
    CancelJobRequest,
    ...
    ListJobsRequest,
    ListPredicateEvaluationsRequest,
    PauseJobRequest,
    ResumeJobRequest,
    RunJobNowRequest,
    UpdateJobRequest,
...
class JobNotFound(LookupError):
    """Raised when a job lookup by id fails."""
...
class JobIntentNotFound(LookupError):
    """Raised when a job intent lookup by id fails."""

    def __init__(self, job_intent_id: str) -> None:
        super().__init__(f"job_intent not found: {job_intent_id}")
        self.job_intent_id = job_intent_id


class DefaultJobService(JobService):
    """Default Job Service implementation with Postgres-backed state."""
        ...
        *,
        ...
        try:
            ...
            job = self._repository.create_job(
                ...
                retry_backoff_strategy=self._settings.default_backoff_strategy.value,
    ...
    def list_predicate_evaluations(
        ...
        *,
        ...
        request, errors = self._validate_request(
            meta=meta,
            model=ListPredicateEvaluationsRequest,
            payload={"job_id": job_id, "limit": limit, "cursor": cursor},
        )
        if errors:
            return failure(meta=meta, errors=errors)
        assert isinstance(request, ListPredicateEvaluationsRequest)
        try:
    ...
    def _evaluate_predicate(
        ...
        *,
        ...
        """Resolve one conditional predicate through a read-only op call."""
        ...
        describe_env = self._execution_service.describe_op(
            meta=predicate_meta,
            op_id=definition.predicate_op_id,
        )
        ...
        if descriptor.approval == "always" or descriptor.effect != "read":
            evaluation = self._repository.create_predicate_evaluation(
                job_id=job.id,
                status=PredicateEvaluationStatus.failed.value,
                predicate_subject=definition.predicate_subject,
                ...
                error_message="predicate op must be read-only",
                trace_id=meta.trace_id,
                created_at=datetime.now(UTC),
            )
            return evaluation, False

        invoke_env = self._execution_service.invoke_op(
            meta=predicate_meta,
            op_id=definition.predicate_op_id,
    ...
    def _get_job_context(self, *, job_id: str) -> tuple[JobRecord, JobIntent]:
        ...
        if intent is None:
            raise JobIntentNotFound(job.job_intent_id)
        return job, intent
    ...
    def _new_invocation_metadata(
        ...
        *,
        ...
        """Build invocation metadata for one Execution call."""
    ...
    def _primary_error(self, errors: list[ErrorDetail]) -> ErrorDetail:
        ...
        if errors:
            return errors[0]
        return dependency_error("op invocation failed", code=codes.DEPENDENCY_FAILURE)
    ...
    def _handle_exception(
        ...
        *,
        ...
        if isinstance(exc, JobIntentNotFound):
            return self._not_found(
                meta=meta,
                entity="job_intent",
                entity_id=exc.job_intent_id,
            )
        if isinstance(exc, JobNotFound):
            return self._not_found(
                meta=meta,
                entity="job",
                entity_id=exc.job_id,
            )
...
def _extract_subject_value(output: Any, subject: str) -> Any:
    """Resolve one slash-or-dot-delimited subject path from op output."""
