...
class EmbeddingSettings:
    ...
    def from_config(cls, config: Mapping[str, Any]) -> "EmbeddingSettings":
        ...
        namespace_strategy = str(
            embedding.get("namespace_strategy", "single_collection")
        )
        ...
        if isinstance(raw_model_dimensions, Mapping):
            model_dimensions = {
                str(key): int(value) for key, value in raw_model_dimensions.items()
            }
    ...
    def validate(self) -> None:
        ...
        if self.namespace_strategy != "single_collection":
            raise ValueError(
                "embedding.namespace_strategy currently only supports 'single_collection'"
            )
