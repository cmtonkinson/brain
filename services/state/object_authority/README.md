...
`services/state/object_authority/` is the authoritative Layer 1 _Service_ for
...
OAS is a State-System _Service_ (`layer=1`, `system="state"`) and declares
ownership of `substrate_seaweedfs` in
`services/state/object_authority/component.py`.

Authority boundaries:
- OAS owns object-key semantics (`b1:sha256:<digest>`), request validation, and
  error mapping.
...
- Callers use `ObjectAuthorityService` (`service.py`) as the canonical
  in-process API.
- OAS validates requests and metadata, computes seeded digest/object key, and
  persists metadata via repository operations.
- OAS persists blob bytes through `SeaweedFSBlobSubstrate`.
...
OAS consumes SeaweedFS substrate settings from
