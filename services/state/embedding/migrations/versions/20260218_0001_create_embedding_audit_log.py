...
def upgrade() -> None:
    """Create Embedding authoritative schema objects."""
    schema = _schema()
    ...
    op.create_table(
        "chunks",
        sa.Column("id", ulid_domain_type(schema), primary_key=True, nullable=False),
        sa.Column("source_id", ulid_domain_type(schema), nullable=False),
    ...
    op.create_table(
        "embeddings",
        sa.Column("chunk_id", ulid_domain_type(schema), nullable=False),
...
def downgrade() -> None:
    """Drop Embedding authoritative schema objects."""
    schema = _schema()
