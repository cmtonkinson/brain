"""Qdrant-backed derived index operations for Embedding Service."""
...
_BOOTSTRAP_POINT_ID = "__bootstrap__"
...
class QdrantEmbeddingBackend:
    ...
    def ensure_collection(self, *, spec_id: str, dimensions: int) -> None:
        ...
        if size is None:
            ...
            substrate.upsert_point(
                point_id=_BOOTSTRAP_POINT_ID,
                vector=[0.0 for _ in range(dimensions)],
                payload={},
            )
            substrate.delete_point(point_id=_BOOTSTRAP_POINT_ID)
            return
        if size != dimensions:
            raise ValueError(
    ...
    def upsert_point(
        ...
        *,
        ...
        if chunk_id == _BOOTSTRAP_POINT_ID:
            raise ValueError(
                f"chunk_id collides with reserved bootstrap point id: {chunk_id!r}"
            )
