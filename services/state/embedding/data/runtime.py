"""Embedding-owned Postgres runtime wiring.

This module composes shared Postgres substrate primitives into a service-local
runtime that enforces Embedding schema scoping via ``search_path``.
"""
...
class EmbeddingPostgresRuntime:
    """Concrete Embedding-owned handle for schema-scoped Postgres access."""
    ...
    def from_settings(cls, settings: CoreRuntimeSettings) -> "EmbeddingPostgresRuntime":
        """Build Embedding DB runtime from typed application settings."""
...
def embedding_postgres_schema() -> str:
    """Resolve the canonical Embedding schema name from component identity."""
