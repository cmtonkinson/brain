"""SQLAlchemy table definitions owned by Embedding Service."""
...
_ACTIVE_SPEC_SINGLETON_MARKER = "active"
...
active_spec = Table(
    ...
    Column(
        "spec_id",
        ulid_domain_type(embedding_postgres_schema()),
    ...
    CheckConstraint(
        f"singleton_marker = '{_ACTIVE_SPEC_SINGLETON_MARKER}'",
...
embeddings = Table(
    ...
    Column("error_detail", Text, nullable=False, server_default=""),
    Column(
        "created_at", DateTime(timezone=True), nullable=False, server_default=func.now()
    ),
    Column(
