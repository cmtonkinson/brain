"""Authoritative Postgres repository for Embedding Service state."""
...
from services.state.embedding.interfaces import EmbeddingRepository

from .schema import (
    _ACTIVE_SPEC_SINGLETON_MARKER,
    active_spec,
    chunks,
    embeddings,
    sources,
...
class PostgresEmbeddingRepository(EmbeddingRepository):
    """SQL repository over Embedding-owned schema tables."""
        ...
        with self._sessions.session() as session:
            row = session.execute(
                select(active_spec.c.spec_id).where(
                    active_spec.c.singleton_marker == _ACTIVE_SPEC_SINGLETON_MARKER
                )
            ).one_or_none()
            if row is None:
                return None
    ...
    def set_active_spec(self, *, spec_id: str) -> None:
        ...
        with self._sessions.session() as session:
            stmt = insert(active_spec).values(
                id=generate_ulid_bytes(),
                singleton_marker=_ACTIVE_SPEC_SINGLETON_MARKER,
