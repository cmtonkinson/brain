"""Behavior tests for Embedding Service authority semantics."""
...
from services.state.embedding.interfaces import IndexSearchPoint
...
def _service() -> tuple[DefaultEmbeddingService, FakeRepository, FakeQdrantIndex]:
...
) -> tuple[DefaultEmbeddingService, FakeRepository, FakeQdrantIndex]:
...
def _create_active_spec(svc: DefaultEmbeddingService) -> EmbeddingSpec:
...
def test_vector_upsert_dependency_failure_marks_embedding_failed(
    ...
    with caplog.at_level(
        logging.WARNING, logger="services.state.embedding.implementation"
    ):
        result = svc.upsert_embedding_vector(
            meta=_meta(),
            chunk_id=chunk.payload.value.id,
...
def test_search_tolerates_non_numeric_chunk_ordinal_in_payload() -> None:
    """Search should default chunk_ordinal to 0 when payload value is non-numeric."""
    svc, _, index = _service()
    spec = _create_active_spec(svc)

    source = svc.upsert_source(
        meta=_meta(),
        canonical_reference="doc://bad-ordinal",
        source_type="note",
        service="ingestion",
        principal="operator",
        metadata={},
    ...
    # Corrupt payload to exercise the (TypeError, ValueError) fallback.
    point_key = (spec.id, chunk.payload.value.id)
    index.payloads[point_key]["chunk_ordinal"] = "not-a-number"
    ...
    assert result.payload is not None
    assert len(result.payload.value) == 1
    assert result.payload.value[0].chunk_ordinal == 0
