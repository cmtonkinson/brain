"""Op registry loading immutable manifests and runtime handlers."""
...
from services.state.vault.service import VaultService


class OpRuntime(Protocol):
    """Runtime helper contract exposed to op handlers."""
    ...
    def invoke_nested(
        ...
        *,
        ...
        """Invoke a nested op under narrowed lineage context."""


OpHandler = Callable[[OpInvocationRequest, OpRuntime], OpExecutionResponse]


@dataclass(frozen=True, slots=True)
class CallTargetContract:
    """Contract for one callable target used by native op manifests."""
...
class OpRegistry:
    """In-memory op registry backed by manifest discovery and handlers."""

    def __init__(self) -> None:
        self._manifests: dict[str, OpManifest] = {}
        self._package_dirs: dict[str, Path] = {}
        self._handlers: dict[str, OpHandler] = {}
        self._slash_commands: dict[str, OpManifest] = {}
    ...
    def discover(
        ...
        *,
        ...
        """Auto-discover ``op.json`` declarations across one or more roots.

        Roots are scanned in order; later roots overlay earlier ones, so an
        op_id present in a later root replaces the same op_id from an
        earlier root. Within a single root, duplicate op_ids remain a hard
        error. Missing roots are skipped silently.
        """
        discovered: dict[str, OpManifest] = {}
        ...
        for root in roots:
            ...
            for manifest_path in self._iter_manifest_paths(root=root):
                ...
                if manifest.op_id in seen_in_root:
                    raise ValueError(f"duplicate op_id discovered: {manifest.op_id}")
                seen_in_root.add(manifest.op_id)
                discovered[manifest.op_id] = manifest
                package_dirs[manifest.op_id] = package_dir
    ...
    def _iter_manifest_paths(self, *, root: Path) -> tuple[Path, ...]:
        ...
        Discovery is recursive, but once a directory is identified as an
        op package (it contains ``op.json``), traversal does
        ...
        for current_root, dir_names, _file_names in os.walk(root, topdown=True):
            ...
            manifest_path = package_dir / "op.json"
    ...
    def _parse_manifest(self, raw: dict[str, Any]) -> OpManifest:
        for required_field in ("effect", "approval"):
            if required_field not in raw:
                raise ValueError(
                    f"op manifest missing required field: {required_field}"
                )
        kind = raw.get("kind")
        if kind in ("native", "mcp"):
            return NativeOpManifest.model_validate(raw)
        if kind in ("logic", "pipeline"):
            return CompoundOpManifest.model_validate(raw)
        raise ValueError(f"Unknown or missing op kind in manifest: {kind}")
    ...
    def _validate_manifest_files(
        ...
        *,
        ...
        if package_dir.name != manifest.op_id:
            raise ValueError("op package directory must match manifest op_id")

        readme_path = package_dir / "README.md"
        if not readme_path.exists():
            raise ValueError(f"op package missing README.md: {manifest.op_id}")

        if isinstance(manifest, CompoundOpManifest):
            if manifest.kind == "logic":
                entrypoint = package_dir / manifest.entrypoint
                if not entrypoint.exists():
                    raise ValueError(f"logic op missing entrypoint: {manifest.op_id}")
                has_tests = any((package_dir / "test").glob("test_*.py"))
                if not has_tests:
                    raise ValueError(f"logic op missing tests: {manifest.op_id}")
            elif manifest.kind == "pipeline" and len(manifest.pipeline) == 0:
                raise ValueError(
                    f"pipeline op must declare pipeline entries: {manifest.op_id}"
                )

    def _validate_closure(self, manifests: dict[str, OpManifest]) -> None:
        manifest_ids = set(manifests.keys())
        for op_id, manifest in manifests.items():
            for dependency in manifest.required_ops:
                if dependency not in manifest_ids:
                    raise ValueError(
                        f"op {op_id} requires unknown dependency {dependency}"
                    )
            if isinstance(manifest, CompoundOpManifest):
                for step in manifest.pipeline:
                    nested = self.pipeline_step(step).op
                    if nested not in manifest_ids:
                        raise ValueError(
                            f"pipeline op {op_id} references unknown op {nested}"
                        )
        ...
        *,
        ...
        for op_id, manifest in manifests.items():
            if isinstance(manifest, NativeOpManifest):
                ...
                if contract is None:
                    raise ValueError(
                        f"native op {op_id} references unknown call target {manifest.call_target}"
                    )
                ...
                if not self._schemas_compatible(
                    ...
                    raise ValueError(
                        f"native op {op_id} input schema does not match call target {manifest.call_target}"
                    )
                ...
                if not self._schemas_compatible(
                    ...
                    raise ValueError(
                        f"native op {op_id} output schema does not match call target {manifest.call_target}"
                    )
                continue
            ...
            first_step = self.pipeline_step(manifest.pipeline[0])
            first = manifests[first_step.op]
            if not self._pipeline_handoff_compatible(
                producer_schema=manifest.input_schema,
                consumer_schema=self.pipeline_step_input_schema(
                    step=first_step,
                ...
                raise ValueError(
                    "pipeline op "
                    f"{op_id} input schema does not satisfy first call target "
                    f"{first.op_id}"
                )
            for index in range(1, len(manifest.pipeline)):
                previous_step = self.pipeline_step(manifest.pipeline[index - 1])
                current_step = self.pipeline_step(manifest.pipeline[index])
                previous = manifests[previous_step.op]
                current = manifests[current_step.op]
                if not self._pipeline_handoff_compatible(
                    ...
                    raise ValueError(
                        f"pipeline op {op_id} has incompatible call targets {previous.op_id} -> {current.op_id}"
                    )
            last = manifests[self.pipeline_step(manifest.pipeline[-1]).op]
            if not self._pipeline_handoff_compatible(
                ...
                raise ValueError(
                    "pipeline op "
                    f"{op_id} output schema is not satisfied by final call target "
                    f"{last.op_id}"
                )
    ...
    def pipeline_step(self, entry: str | PipelineStep) -> PipelineStep:
        """Normalize one pipeline entry to the object form."""
        return PipelineStep.from_entry(entry)

    def pipeline_step_input_schema(
        self,
        *,
        step: PipelineStep,
        consumer_schema: dict[str, Any] | None,
        ...
        if consumer_schema is None:
            raise ValueError(
                f"pipeline step {step.op} declares input_mapping but target has no input schema"
            )
        if not isinstance(consumer_schema, dict):
            raise ValueError(
                f"pipeline step {step.op} input schema must be an object when using input_mapping"
            )
        ...
        if not isinstance(properties, dict):
            raise ValueError(
                f"pipeline step {step.op} input schema must declare object properties when using input_mapping"
            )
        ...
        for consumer_field, producer_field in step.input_mapping.items():
            ...
            if not isinstance(field_schema, dict):
                raise ValueError(
                    f"pipeline step {step.op} input_mapping references unknown input field {consumer_field}"
                )
    ...
    def _discover_native_service_targets(self) -> dict[str, CallTargetContract]:
        ...
        services: tuple[tuple[str, type[Any]], ...] = (
            ...
            ("service_recall", RecallService),
            ("service_object", ObjectService),
            ("service_vault", VaultService),
    ...
    def _schema_from_signature(
        ...
        for parameter in signature.parameters.values():
            ...
            prop_schema = OpRegistry._schema_from_annotation(annotation)
    ...
    @staticmethod
    def _schema_from_return_annotation(annotation: Any) -> dict[str, Any] | None:
        if annotation is inspect.Signature.empty:
            return None
        # Unpack Envelope[T] to T — try stdlib generics first, then Pydantic metadata
        ...
        if (
            ...
            return OpRegistry._schema_from_annotation(args[0])
        return OpRegistry._schema_from_annotation(annotation)
    ...
    def _schema_from_annotation(annotation: Any) -> dict[str, Any]:
        ...
        # typing.Any → unconstrained JSON Schema (no type restriction)
        if annotation is Any:
            return {}

        if isinstance(annotation, type) and issubclass(annotation, Enum):
            # StrEnum/IntEnum values serialize to a single JSON-Schema scalar;
            # expose member values via ``enum`` so op callers know the options.
            base_type = "string"
        ...
        if origin is list or origin is Sequence:
            return {
                "type": "array",
                "items": OpRegistry._schema_from_annotation(args[0]),
            }

        if origin is tuple:
            # Homogeneous variadic tuples (``tuple[T, ...]``) map to JSON arrays.
            # Heterogeneous fixed-arity tuples have no JSON Schema equivalent at
            # the call_target boundary; fall back to an untyped array stub.
        ...
        if origin is Union or origin is UnionType:
            return {"anyOf": [OpRegistry._schema_from_annotation(arg) for arg in args]}

        if isinstance(annotation, ForwardRef):
            # Forward references can't be resolved here; emit an object stub
            # so the contract validator has something to compare against.
            return {"type": "object"}

        if hasattr(annotation, "__name__"):
            # Named classes (Pydantic models, dataclasses, etc.) produce a
            # coarse object stub; the manifest's detailed schema takes precedence
            # during call_target contract validation.
    ...
    def register_handler(
        ...
        *,
        ...
        """Register one runtime handler for an existing op manifest."""
        self._handlers[op_id] = handler

    def resolve_manifest(self, *, op_id: str) -> OpManifest | None:
        """Resolve one op manifest by package op identifier."""
        return self._manifests.get(op_id)

    def resolve_package_dir(self, *, op_id: str) -> Path | None:
        """Resolve the on-disk package directory for one discovered op."""
        return self._package_dirs.get(op_id)

    def resolve_handler(self, *, op_id: str) -> OpHandler | None:
        """Resolve one op handler by package op identifier."""
        return self._handlers.get(op_id)

    def list_manifests(self) -> tuple[OpManifest, ...]:
        """Return all registered op manifests in stable order."""
        return tuple(self._manifests[k] for k in sorted(self._manifests))

    def count(self) -> int:
        """Return number of discovered op manifests."""
