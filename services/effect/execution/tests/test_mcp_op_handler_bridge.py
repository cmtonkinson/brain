...
class TestMcpOpId:
    ...
    def test_simple(self) -> None:
        assert mcp_op_id("eventkit", "list_events") == "eventkit--list-events"

    def test_hyphens_in_tool(self) -> None:
        assert mcp_op_id("fs", "read-file") == "fs--read-file"

    def test_underscores_normalized(self) -> None:
        assert mcp_op_id("my_server", "my_tool") == "my-server--my-tool"

    def test_uppercase_lowered(self) -> None:
        assert mcp_op_id("FS", "ReadFile") == "fs--readfile"

    def test_special_chars_stripped(self) -> None:
        assert mcp_op_id("fs!", "tool@v2") == "fs--tool-v2"
