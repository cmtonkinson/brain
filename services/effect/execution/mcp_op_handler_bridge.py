"""MCP Op handler bridge: builds OpHandlers for MCP tool invocations."""
...
def build_mcp_op_handler(
    *,
    ...
    """Build an OpHandler that delegates to an MCP tool via the adapter.

    The returned closure captures ``server_id``, ``tool_name``, and the
    adapter reference.  At invocation time it calls
    ``adapter.call_tool()`` and normalizes the MCP content result into an
    ``OpExecutionResponse``.
    """
...
def mcp_op_id(server_id: str, tool_name: str) -> str:
    """Derive a kebab-case op_id from server_id and tool_name.
...
def normalize_mcp_content(content: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Normalize MCP tool content into an Execution-compatible output dict.
