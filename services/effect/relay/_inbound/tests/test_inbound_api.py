"""HTTP route adapter tests for Relay inbound service."""
...
class _FakeRelayInboundService(RelayInboundService):
    """Programmable Relay inbound fake for Core route-adapter tests."""
...
def _meta():
    """Build valid envelope metadata for Relay inbound route-test responses."""
...
def _client() -> tuple[TestClient, _FakeRelayInboundService]:
    """Create one FastAPI test client with the Relay inbound route adapter mounted."""
    app = create_app()
    router = APIRouter()
...
def test_poll_operator_instruction_route_forwards_request_and_returns_payload() -> None:
    ...
    assert service.poll_calls == [
        _PollCall(wait_timeout_seconds=1.5, source="assistant", principal="assistant")
    ]
