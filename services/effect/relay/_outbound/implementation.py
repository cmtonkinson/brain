"""Concrete Relay outbound service implementation."""
...
from services.effect.relay._outbound.component import SERVICE_COMPONENT_ID
...
from services.effect.relay._outbound.validation import (
...
class DefaultRelayOutboundService(RelayOutboundService):
    """Relay outbound implementation with dedupe, batching, and rate limits."""
    ...
    def health(self, *, meta: EnvelopeMeta) -> Envelope[HealthStatus]:
        """Return Relay outbound self-readiness state."""
        try:
            validate_meta(meta)
        ...
        *,
        ...
        if self._cache_service is None:
            return failure(
                ...
                errors=[
                    dependency_error(
                        "cache unavailable for console response polling",
    ...
    def _normalize_conversational_memory(
        *,
        ...
        """Normalize one optional conversational-memory payload into Recall-owned shape."""
    ...
    def _persist_approval_timestamp_correlation(
        ...
        *,
        ...
        result = self._cache_service.set_value(
            meta=meta,
            component_id=str(SERVICE_COMPONENT_ID),
    ...
    def _load_approval_timestamp_correlation(
        ...
        *,
        ...
        result = self._cache_service.get_value(
            meta=meta,
            component_id=str(SERVICE_COMPONENT_ID),
            key=self._approval_timestamp_cache_key(
                channel=channel,
    ...
    def _persist_conversational_outbound(
        ...
        *,
        ...
        """Persist one actually sent conversational outbound into Recall."""
        if conversational_memory is None:
            return None
        ...
        candidate = self._recall_service.record_outbound_candidate(
            meta=candidate_meta,
        ...
        delivery = self._recall_service.record_outbound_delivery(
            meta=candidate_meta,
            session_id=conversational_memory.session_id,
    ...
    def _deliver_via_console(
        ...
        *,
        ...
        if self._cache_service is None:
            return None, dependency_error(
                "cache unavailable for console delivery",
                code=codes.DEPENDENCY_UNAVAILABLE,
            )

        timestamp_ms = int(time.time() * 1000)
        result = self._cache_service.push_queue(
            meta=new_meta(
