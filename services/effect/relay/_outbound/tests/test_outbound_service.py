"""Behavior tests for Relay outbound service implementation."""
...
from services.state.cache.domain import CacheEntry
...
class _FakeSignalAdapter(SignalAdapter):
    """In-memory Signal adapter fake for Relay outbound behavior tests."""
...
class _FakeCacheService(CacheService):
    """In-memory Cache fake for approval correlation persistence tests."""
...
class _FakeRecallService(RecallService):
    """In-memory Recall fake for conversational outbound persistence tests."""
...
def _meta():
    """Build valid envelope metadata for Relay outbound tests."""
    return new_meta(
...
def _service() -> tuple[
    ...
    """Build Relay outbound with in-memory dependencies for tests."""
    adapter = _FakeSignalAdapter()
    cache = _FakeCacheService()
    memory = _FakeRecallService()
    service = DefaultRelayOutboundService(
...
def test_route_notification_persists_sent_conversational_outbound_to_mas() -> None:
    """Sent notifications on conversational channels should be recorded in Recall."""
    service, adapter, _cache, memory = _service()
...
def test_route_notification_skips_mas_for_non_conversational_channel() -> None:
    """Sent notifications on non-conversational channels should not hit Recall."""
    service, adapter, _cache, memory = _service()
...
def test_flush_batch_persists_when_sent_with_stable_conversational_context() -> None:
    """Flushed conversational batches should write one emitted outbound to Recall."""
    service, adapter, _cache, memory = _service()
...
def test_resolve_approval_notification_token_fails_when_cache_lookup_fails() -> None:
    """Token lookup should surface dependency failure when Cache lookup fails."""
    service, _adapter, cache, _memory = _service()
    cache.fail_get = True
...
def test_resolve_approval_notification_token_returns_none_when_cache_value_missing() -> (
    ...
    """Token lookup should treat empty Cache payload values as a cache miss."""
    service, _adapter, cache, _memory = _service()
