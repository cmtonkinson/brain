"""Concrete Language Service implementation."""
...
from services.effect.language.validation import (
    ChatBatchRequest,
    ChatRequest,
...
# Anthropic: 1.25x write cost → 0.25x premium; 0.10x read cost → 0.90x savings
...
class DefaultLanguageService(LanguageService):
    """Default Language implementation backed by the native LLM adapter resource."""
    ...
    def chat(
        ...
        *,
        ...
        try:
            with _language_generation_observation(
                meta=meta,
                operation="chat",
                ...
                _complete_language_generation_observation(
                    span=span,
                    outcome="success",
    ...
    def chat_batch(
        ...
        *,
        ...
        self._append_call_audit(
            meta=meta,
            provider=resolved.provider if first is None else first.provider,
            model=resolved.model if first is None else first.model,
            profile=request.profile.value,
            operation="chat_batch",
            request_phase="initial",
            outcome_kind="final",
    ...
    def get_token_usage_by_trace(
        ...
        *,
        ...
        """Aggregate persisted call_audits usage for one trace_id."""
        try:
            validate_meta(meta)
        ...
        if normalized == "":
            return failure(
                meta=meta,
                errors=[validation_error("trace_id is required")],
            )
        totals = self._audit_repository.sum_token_usage_by_trace(trace_id=normalized)
        return success(meta=meta, payload=totals)
    ...
    def health(self, *, meta: EnvelopeMeta) -> Envelope[HealthStatus]:
        """Return Language-level readiness with adapter probe result."""
        try:
            validate_meta(meta)
        except ValueError as exc:
            return failure(
    ...
    def _resolve_embed_profile(self, *, profile: EmbeddingProfile) -> _ResolvedProfile:
        ...
        if profile is EmbeddingProfile.OP_EMBEDDING:
            return _from_settings(self._settings.op_embedding)
    ...
    def _validate_request(
        ...
        *,
        ...
        try:
            ...
            return None, [
                validation_error(
                    f"{'.'.join(str(p) for p in e['loc']) or 'payload'}: {e.get('msg', 'invalid value')}",
    ...
    def _append_call_audit(
        ...
        *,
        ...
        """Append one provider-bound Language call audit row and return the stored row."""
    ...
    def _append_turn_cache_hop(
        ...
        *,
        ...
        """Append one per-hop cache telemetry row for a tool-capable Language call."""
        request_json = None if raw_call is None else _provider_request_json(raw_call)
...
def _from_settings(
    settings: LanguageProfileSettings | LanguageEmbeddingProfileSettings,
) -> _ResolvedProfile:
    """Convert required profile settings into resolved call-time tuple."""
    dimensions = (
        settings.dimensions
        if isinstance(settings, LanguageEmbeddingProfileSettings)
        else None
    )
...
def _request_phase_for_inference_request(request: InferenceRequest) -> str:
    ...
    for event in request.live_events:
        if isinstance(
            event, (InferenceToolCallBatchEvent, InferenceToolResultBatchEvent)
        ):
            return "tool_followup"
    return "initial"


def _tool_chat_outcome_kind(*, result: AdapterToolChatResult) -> str:
...
def _langfuse_observation_input(
    ...
    """Return compact IR-shaped Langfuse input for one tool-capable Language call."""
    return {
        "request_phase": request_phase,
...
def _langfuse_chat_observation_input(
    *, system_prompt: str, prompt: str, request_phase: str
    ...
    """Return compact Langfuse-facing input for one plain chat Language call."""
...
@contextmanager
def _language_generation_observation(
    *,
    meta: EnvelopeMeta,
    operation: str,
    provider: str,
    ...
    with tracer.start_as_current_span(f"lms.{operation}") as span:
        ...
        try:
            ...
            _record_language_generation_error_on_span(span=span, exc=exc)
            raise


def _complete_language_generation_observation(
    *,
    span: object | None,
    outcome: str,
    provider: str,
...
def _record_language_generation_error_on_span(*, span: object, exc: Exception) -> None:
