"""Language Service-owned Postgres runtime wiring."""
...
class LanguagePostgresRuntime:
    """Concrete Language handle for schema-scoped Postgres access."""
    ...
    @classmethod
    def from_settings(cls, settings: CoreRuntimeSettings) -> "LanguagePostgresRuntime":
        """Build Language DB runtime from typed application settings."""
        postgres_config = resolve_postgres_settings(settings)
        engine = create_postgres_engine(postgres_config)
        session_factory = create_session_factory(engine)
        schema = language_model_postgres_schema()
        return cls(
            engine=engine,
...
def language_model_postgres_schema() -> str:
    """Resolve canonical Language schema name from component identity."""
