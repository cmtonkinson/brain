"""Tests for Attention Router HTTP API routes."""
...
from services.action.attention_router.api import register_routes
...
class _FakeAttentionRouterService(AttentionRouterService):
    """Programmable Attention Router fake for API route tests."""
...
def _client() -> tuple[TestClient, _FakeAttentionRouterService]:
    app = create_app()
    router = APIRouter()
