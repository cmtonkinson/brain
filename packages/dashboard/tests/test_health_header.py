...
from packages.dashboard.widgets.health_header import _render_health, _COMPONENT_ORDER
...
def test_render_health_component_order():
    components = [_health(name, "ok") for name in _COMPONENT_ORDER]
    text = _render_health(components)
    # all 7 component names present in order
    positions = [text.index(name) for name in _COMPONENT_ORDER]
    assert positions == sorted(positions)


def test_render_health_two_char_tokens_only():
    components = [_health("core", "ok"), _health("agent", "no")]
    text = _render_health(components)
