...
The _Public APIs_ are canonical. L2 access is provided by a small published HTTP
...
using the published HTTP surface. All L2 _Actors_ must be built on the _Brain
Core SDK_.

The SDK contains no business logic; it simply exists as an access layer across
process boundaries to the published subset of L1 _Service_ _Public APIs_.
