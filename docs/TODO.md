# TODO
- BUG: why are summaries no longer happening? Because Letta?
- Add total e2e delay as a metric for observability
- There are a lot of things in src/config.py that should be in a config file
- Integrate with Tactiq API for automatic pull of transcriptions into Obsidian
- Add MCP server integration for https://github.com/XD3an/python-sequential-thinking-mcp
- Add MCP server integration for https://github.com/firecrawl/firecrawl-mcp-server
- Is Obsidian being chunked & embedded for search?
- Whitelist incoming numbers (duh)
