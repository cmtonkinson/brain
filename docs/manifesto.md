...
Its purpose is to:
* *exploit knowledge* whenever useful,
* translate intent into *successful action*,
...
Because memories are written *for the human first* (and the machine second),
...
------------------------------------------------------------------------
## Ops, Not Scripts
Reusable work is expressed as *Ops*: the units of
compounding leverage within the system. An *Op*:
...
*No component may bypass the Relay.* In fact, there are automated test gates
...
* *Ops* cannot write durable memory directly.
