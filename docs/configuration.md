...
BRAIN_POSTGRES__POOL_SIZE=10
BRAIN_EMBEDDING__QDRANT_URL=http://localhost:6333
```
...
| Key | Default | Description |
|---|---|---|
| `url` | `postgresql+psycopg://brain:brain@postgres:5432/brain` | SQLAlchemy-style connection URL. Override with `BRAIN_POSTGRES__URL`. |
...
------------------------------------------------------------------------
## `embedding`
Settings for the Embedding Authority Service and its Qdrant backend.

| Key | Default | Description |
|---|---|---|
| `qdrant_url` | `http://qdrant:6333` | Base URL of the Qdrant vector search instance. |
